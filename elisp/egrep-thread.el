...
(defun ergrep () ;; elisp recursive grep
  ...
  (let* ((string (read-string "Search for: "))
    ...
    (out-buffer (generate-new-buffer
                 (format "grep-recursive %s %s" string dirname))))
    
    (setq ergrep-thread (lambda ()
      ...
      (save-excursion
        (with-current-buffer out-buffer ;; header of the file
          (insert (format "grep-recursive %s %s" string dirname))
          (make-thread ergrep-thread)
          ))))
